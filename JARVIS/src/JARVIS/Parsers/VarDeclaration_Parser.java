...
public class VarDeclaration_Parser extends Parser
{
    ...
    public Block Parse() {
        ...
        {
            Block sb = superBlock;
            while((superBlock.getType()!=BlockType.METHOD))
            {
                System.out.println("OOF");
                superBlock = superBlock.getSuper();
            }
            if(superBlock.getType() == BlockType.METHOD){
            Method_Block mb = (Method_Block)superBlock;
                if(mb.getName().equals("main"))
                    isMain = true;
            }
            superBlock = sb;
        }
        ...
        if(first.getType() == TokenType.DATA_TYPES)
        {
                ...
                if(chk == 0)
                {
                    ...
                    if(next.getToken().equals("=") && varType!=DataType.VOID)
                    {
                        ...
                        if(next.getType() == TokenType.IDENTIFIER)
                        {
                            ...
                            if((isMain && inJARVIS) || !init)
                            {
                                didParse = false;
                                return (lastBlock = new VarDeclaration_Block(superBlock,block,BlockType.ERROR,null,null,isInit));
                            }
                            if(chk==1 && init)
                            {
                                ...
                                didParse = true;
                                return (lastBlock = new VarDeclaration_Block(superBlock,block,BlockType.VAR_DECLARATION,varType,name,isInit));
                            }
                            else
                            {
                                return (lastBlock = new VarDeclaration_Block(superBlock,block,BlockType.ERROR,null,null,isInit));
            ...
            {
                didParse = false;
                return (lastBlock = new VarDeclaration_Block(superBlock,block,BlockType.ERROR,null,null,false));
            }
        }
